...
namespace Bottles
{
    ...
    public class BottleInfo : IBottleInfo
    {
        ...
        void IBottleInfo.LoadAssemblies(IAssemblyRegistration loader)
        {
            //double double dispatch - getting deep
            _assemblies.Each(a => a.Load(loader));
        }

        void IBottleInfo.ForFolder(string folderName, Action<string> onFound)
        {
            _files.ForFolder(folderName, onFound);
        }

        void IBottleInfo.ForData(string searchPattern, Action<string, Stream> dataCallback)
        {
            _files.ForData(searchPattern, dataCallback);
        }

        IEnumerable<Dependency> IBottleInfo.GetDependencies()
        {
